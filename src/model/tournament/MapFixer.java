...
public class MapFixer {
	...
	public static void pc(Task t) {
		...
		System.out.println(t.getChampions().size());

		for (Champion c : t.getChampions())
			System.out.println(c + "  " + ((Wizard) c).getName());
		...
		if (g != null)
			t.setCurrentChamp(g);
		if (g != null && r != null && s != null && h != null) {
			t.getChampions().clear();
	...
	public static void printMap(Task t) {
		...
		for (int i = 0; i < m.length; i++) {
			for (int j = 0; j < m[i].length; j++) {
				System.out.print(m[i][j].getClass().getSimpleName()
						.substring(0, 4));
			}
			System.out.println();
		}
	...
	public static void main(String[] args) throws IOException {
		...
		t.beginTournament();
		printMap(t.getFirstTask());
		// t.onFinishingFirstTask(t.getChampions());
		// printMap(t.getSecondTask());
		// t.getSecondTask().getWinners().add(getSly(t.getSecondTask()));
		// t.getSecondTask().getWinners().add(getRav(t.getSecondTask()));
		...
		// t.onFinishingSecondTask(t.getSecondTask().getWinners());
		// printMap(t.getThirdTask());

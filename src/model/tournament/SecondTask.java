...
public class SecondTask extends Task {
	...
	public void moveForward() throws IOException, OutOfBordersException,
		...
		if (next instanceof TreasureCell
			...
			cont.ChampWonSecond(current,location);
	...
	public void finalizeAction() throws IOException {
		...
		if (getAllowedMoves() == 0) {
			...
			if (!(current instanceof HufflepuffWizard && isTraitActivated())&&(getChampions().contains(getCurrentChamp())))
					encounterMerPerson();
			else
				JOptionPane.showMessageDialog(null, "merpeople won't attack, thanks to your trait!");
	...
	public Object onRavenclawTrait() throws InCooldownException {
		...
		String s1 = result.get(0).toString()+".png";
		String s2= null;
		if(result.size()>1)s2= result.get(1).toString()+".png";
		new Turn(s1,s2,null);
		return result;

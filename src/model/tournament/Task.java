...
public abstract class Task implements WizardListener {
	...
	public void moveForward() throws IOException, OutOfBordersException, InvalidTargetCellException {
		...
		if (getMap()[newLocation.x][newLocation.y] instanceof CollectibleCell) {
			Potion p = (Potion) ((CollectibleCell) getMap()[newLocation.x][newLocation.y]).getCollectible();
			current.getInventory().add(p);
			cont.showPotion(newLocation);
			JOptionPane.showMessageDialog(null,
					"Hooray! " + p.getName() + " (potion) with " + p.getAmount()
							+ " IP recovery points will be added to your inventory",
		...
		cont.move(location,newLocation);
		finalizeAction();
	...
	public void castDamagingSpell(DamagingSpell spell, Direction d) throws IOException, OutOfBordersException,
		...
		if (map[target.x][target.y] instanceof ObstacleCell) {
			...
			cont.updateobs(target);
			else
				cont.updatemer(target);
			...
			if (this instanceof ThirdTask && opponent instanceof HufflepuffWizard) {
				...
				JOptionPane.showMessageDialog(null, "The hufflepuff wizard will be affected by half the damage only!");
			} else {
			...
			if (newHp <= 0) {
				...
				cont.champAffected((Champion)opponent);
				cont.ChampDied(opponent);
			} else {
	...
	public void castRelocatingSpell(RelocatingSpell spell, Direction d, Direction t, int range)
		...
		if (map[target.x][target.y] instanceof ObstacleCell) {
			cont.move(target,new Point(newX,newY));
			Obstacle obstacle = ((ObstacleCell) map[target.x][target.y]).getObstacle();
	...
	public void castHealingSpell(HealingSpell spell) throws IOException, InCooldownException, NotEnoughIPException {
		...
		current.setHp(newHp);
		useSpell(spell);
		cont.champAffected(currentChamp);
		finalizeAction();
	...
	public void endTurn() throws IOException {
		...
		String name = ((Wizard)currentChamp).getName();
		allowedMoves = 1;
	...
	public void usePotion(Potion p) {
		...
		cont.champAffected((Champion)current);
	...
	public void onSlytherinTrait(Direction d)
		...
		cont.move(location, new Point(newX,newY));
		traitActivated = true;
		cont.updateCur();
		finalizeAction();
